...
/*
	...
	- macOS
	- Linux
	- Windows if the file path contains ASCII characters only

	This code is not really meant for production purposes;
	its known limitations include at least the following:
	- does not support files with more than two channels
	- does not support compressed audio formats
	- does not support files with more than "fmt", "PAD" and "data" chunks
	- does not support files with "PAD" chunks longer than 9000 bytes
	- on Windows, runs only if the file path is ASCII-only
	- sends messages only to stderr

	Nevertheless, "simplistic mono or stereo 16-bit PCM WAV" is a format
	into which you will be able to convert most more complicated formats
	with the help of audio file converters,
	such as those available in iTunes, Windows Media Player, or Praat.
 */
